...
const CongestionMap = () => {
  ...
  return (
    <div className="relative overflow-hidden bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl p-5 shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col h-full group">
      <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent pointer-events-none" />

      <div className="flex items-center gap-2 mb-4 relative z-10">
        <div className="w-1.5 h-4 rounded-full bg-primary animate-pulse" />
        <h3 className="text-sm font-semibold text-white uppercase tracking-wider">Live Congestion Map</h3>
      </div>

      <div className="relative w-full h-[300px] rounded-lg overflow-hidden grid-bg border border-white/5 shadow-inner">
          ...
          zones.map((z, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute top-14 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md border border-white/10 rounded-lg px-3 py-2 opacity-0 group-hover/zone:opacity-100 transition-opacity whitespace-nowrap z-20 shadow-xl pointer-events-none">
                  <p className="text-sm font-semibold text-white mb-0.5">{z.label}</p>
                </div>
              </motion.div>
            );
          })
      ...
      <div className="flex gap-5 mt-5 justify-center relative z-10 flex-wrap">
        <div className="flex items-center gap-2 bg-white/5 rounded-full px-3 py-1 border border-white/5">
          ...
          <span className="text-xs text-gray-300 font-medium">Low Traffic</span>
        </div>
        <div className="flex items-center gap-2 bg-white/5 rounded-full px-3 py-1 border border-white/5">
        ...
        <div className="flex items-center gap-2 bg-white/5 rounded-full px-3 py-1 border border-white/5">
          ...
          <span className="text-xs text-gray-300 font-medium">High Traffic</span>
        </div>
      </div>
    </div>
  );
